...
class ClientApp:
    def __init__(self, root):
        ...
        self.server_status_label.grid(column=0, row=4, padx=10, pady=5)

        ttk.Button(root, text="Add Device", command=self.add_device).grid(column=0, row=5, columnspan=2, pady=10)

        ttk.Button(root, text="Ping Server", command=self.check_server).grid(column=0, row=6, columnspan=2, pady=10)

        ttk.Button(root, text="Send Heartbeat", command=self.send_heartbeat).grid(column=0, row=7, columnspan=2, pady=10)

        ttk.Button(root, text="Quit", command=root.quit).grid(column=0, row=8, columnspan=2, pady=10)
